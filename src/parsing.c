...
void parse_error (struct Bot *b, char *f) {
	...
	if ( b->error_connects == b->max_connects) {
		...
		if (b->current_server == 0) { b->current_server = no_IRCServers(b); }
	...
	while (ok!=1) {
		...
		if ( attempts>b->max_connects ) {
			...
			b->current_server--;
			if (dns_fail!=1) { do_bot_log (b, "Maximum retires exceeded. Trying the next server.") ; }
			attempts=0;
			dns_fail=0;
